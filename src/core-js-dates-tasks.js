...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  const firstDate = new Date(dateStart).getTime();
  const secondDate = new Date(dateEnd).getTime();
  return Math.ceil(Math.abs(secondDate - firstDate) / (1000 * 3600 * 24) + 1);
}
...
function getWeekNumberByDate(date) {
  ...
  return (
    1 +
    Math.ceil((timestamp - targetDate.getTime()) / (24 * 60 * 60 * 1000 * 7))
  );
}
...
function getNextFridayThe13th(date) {
  ...
  do {
    result.setMonth(result.getMonth() + 1, 13);
